...
def testGenerator(test_path,num_image,target_size, offset_height, offset_width, blank):
    if num_image == 1:
...
def test():
        ...
        testGene = testGenerator(test_path, num_image=num_img, target_size=target, \
                                offset_height=target[0]/2, offset_width=target[1]/2, blank=blank_image)
        ...
        testGene = testGenerator(test_path, num_image=num_img, target_size=target, \
                                offset_height=target[0]/5, offset_width=target[1]/5, blank=blank_image)
        results = model.predict_generator(testGene, num_img)
